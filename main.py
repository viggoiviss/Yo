...
@client.event
async def on_ready():
    await client.change_presence(activity=discord.Activity(type=discord.ActivityType.watching, name="SneaX#9999"))
    print(f"[CONNECTION] {client.user} ({client.user.id})")
...
class NitroProofCustom(discord.ui.Modal, title='Fake Nitro Proof System'):
    nitrotype = discord.ui.TextInput(
        ...
        placeholder='Example: classic/boost/promo',
        required=True,
        max_length=7,
    )
    ...
    receivertext = discord.ui.TextInput(
        ...
        placeholder="Example: thank you so much.",
        required=True,
    )
